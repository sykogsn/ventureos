...
`JobOrchestrator`, `AuditLog`, workforce execution records, agent definitions, agent instances, workforce runs, workforce approvals, workforce verifications, and Qualora evidence assessments persist through the same SQLite connection. They call `getDb()` / `getClient()` per operation so `resetPersistenceLifecycle()` does not leave them bound to a closed client. Handlers stay in-process and are not stored. Interrupted `running` jobs are failed with `interrupted-by-restart` on the first `processDue` of a new orchestrator instance; they are not replayed. Interrupted `running` workforce executions are failed with `INTERRUPTED` when a new execution store is constructed; they are not replayed. Interrupted workforce runs in `reasoning` are failed `INTERRUPTED` and are not model-replayed. Runs waiting for approval remain `awaiting_approval`. Runs in `verifying` survive restart and are not completed merely because a Sprint 5 execution row succeeded. If execution already succeeded, recovery moves toward verification and never re-executes. The 15-second `jobs.processDue` tick may then call `orchestrator.recover()`; recovery is bounded and deterministic, performs no model reasoning, and never re-executes a completed or interrupted business action. Verify jobs already queued or running, including future `runAt`, count as active and are not double-enqueued. A live observe nonce is not released while a verify job is active. Schema generation 7 adds optional `implementation_id` / `implementation_version` on execution and verification rows and bounded optional `external_reference` on executions. Schema generation 8 adds optional Venture-agnostic `evidence_json` / `citations_json` on workforce runs (bounded ModelPort pack, not Qualora columns) and the Qualora-owned `qualora_evidence_assessments` table for AI-generated, unconfirmed, proposed evidence-gap rows. Schema generation 9 adds Qualora-owned `qualora_evidence_assessment_reviews` for one final human CONFIRMED or DISMISSED decision bound to the exact assessment fingerprint. CONFIRMED means only that an authorised human confirms the proposed assessment is an appropriate assessment of the supplied evidence; it is not Sprint 7 VERIFIED and not a compliance or non-compliance verdict. DISMISSED preserves the original proposed assessment. Completing a run with `no_action` does not mean compliant, supported, or regulator-accepted; it means only that this WorkforceRun did not propose an evidence gap. A Sprint 7 `VERIFIED` outcome means the intended Qualora assessment row was independently observed with the expected bound fields. It does not mean the AI judgement is correct. Job payload JSON and execution `outcome_json` are parsed fail-closed. Audit is append-only and must not store VIC snapshots, model prompts, or evidence packs. Workforce execution rows store argument hashes, not model prose. Verification evidence is bounded JSON on the verification row (8 KiB ceiling) and is not copied into audit metadata or the redacted inspector. Qualora assessment rows store cited evidence ids and a bounded summary, not full evidence excerpts or chain-of-thought. Qualora review audit records the human actor, assessment id, and decision; it does not store excerpts or assessment payloads.
