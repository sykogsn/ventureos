...
describe("Engineering Intelligence", () => {
  ...
  it("projects the current ECE catalogue as a process baseline without inventing a trend", () => {
    ...
    assert.ok(catalogue.cycles.some((cycle) => cycle.id === "ECE-003"));
    ...
    assert.match(intelligence.process.certificationFirstPass.label, /1\/2/);
    assert.match(intelligence.process.certificationFirstPass.label, /rate withheld/);
    assert.equal(intelligence.process.certificationFirstPass.held, 1);
    assert.equal(intelligence.process.certificationFirstPass.known, 2);
    ...
    assert.equal(intelligence.process.cleanExit.held, 3);
    assert.equal(intelligence.process.cleanExit.known, 3);
